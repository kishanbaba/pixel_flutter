...
import 'package:pixel_app/Employee/ViewModel/employee_leave_update_veiwModel.dart';
...
class EmployeeLeaveUpdateScreen extends StatefulWidget {
  const EmployeeLeaveUpdateScreen({this.leaveDataList});

  final LeaveDataList? leaveDataList;

  @override
  State<EmployeeLeaveUpdateScreen> createState() =>
      _EmployeeLeaveUpdateScreenState();
}

class _EmployeeLeaveUpdateScreenState extends State<EmployeeLeaveUpdateScreen> {
  ...
  EmployeeLeaveUpdateViewModel employeeLeaveUpdateViewModel =
      Get.put(EmployeeLeaveUpdateViewModel());
  ...
  String selectedLeaveTypeNameHint = '';
  String selectedDurationNameHint = '';
  ...
  void getSelectedLeaveData() {
    leaveNote.text = widget.leaveDataList!.leaveNm!;
    leaveDate.text =
        '${widget.leaveDataList!.leaveDt!.day}-${widget.leaveDataList!.leaveDt!.month}-${widget.leaveDataList!.leaveDt!.year}';
    selectedDurationId = widget.leaveDataList!.durationId!;
    selectedLeaveTypeId = widget.leaveDataList!.leaveTypeId!;

    selectedDateString = formatISOTime(date: widget.leaveDataList!.leaveDt!);
    print('selectedDateStringselectedDateString===${selectedDateString}');
  }
                ...
                GetBuilder<EmployeeLeavesMasterViewModel>(
                  builder: (controller) {
                    if (controller.apiResponse.status == Status.COMPLETE) {
                      ...
                      response.data!.duration!.forEach((element) {
                        if (element.durationId == selectedDurationId) {
                          selectedDurationNameHint = element.durationNm ?? '';
                        }
                      });

                      response.data!.leaveType!.forEach((element) {
                        if (element.leaveTypeId == selectedLeaveTypeId) {
                          selectedLeaveTypeNameHint = element.leaveTypeNm ?? "";
                        }
                      });
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            DropdownButtonFormField(
                              ...
                              //   return null;
                              // },
                              hint: selectedLeaveTypeNameHint.isNotEmpty
                                  ? Text(
                                      "${selectedLeaveTypeNameHint}",
                            ...
                            DropdownButtonFormField(
                              ...
                              //   return null;
                              // },
                              hint: selectedDurationNameHint.isNotEmpty
                                  ? Text(
                                      "${selectedDurationNameHint}",
                                      style: const TextStyle(
                                          color: Colors.black,
                                          fontFamily: "Roboto",
                                          fontSize: 13),
                ...
                GestureDetector(
                  onTap: () async {
                    if (_formKey.currentState!.validate()) {
                      EmployeeLeaveUpdateRequestModel leaveAddReq =
                          EmployeeLeaveUpdateRequestModel();
                      leaveAddReq.leaveId = widget.leaveDataList!.leaveId;
                      ...
                      await employeeLeaveUpdateViewModel
                          .employeeLeaveUpdateViewModel(leaveAddReq);
                      if (employeeLeaveUpdateViewModel.apiResponse.status ==
                          Status.COMPLETE) {
                        EmployeeLeaveUpdateResponseModel response =
                            employeeLeaveUpdateViewModel.apiResponse.data;
